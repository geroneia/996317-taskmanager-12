...
export default class TaskEdit extends SmartView {
  constructor(task = BLANK_TASK) {
    ...
    // устанавливает flatpickr при инициализации
    this._setDatepicker();
  }

  // у родителя перегружается метод removeElement, чтобы удалился ненужный календарь
  removeElement() {
    super.removeElement();
    if (this._datepicker) {
  ...
  restoreHandlers() {
    this._setInnerHandlers();
    // устанавливает flatpickr в случае перерендеринга компонента
    this._setDatepicker();
  ...
  _setDatepicker() {
    ...
    // если поле выбора даты доступно для заполнения, инициализируется flatpickr
  ...
  _dueDateToggleHandler(evt) {
    ...
    this.updateData({
      ...
      // когда показывается выбор даты, isRepeating должно быть false
  ...
  _dueDateChangeHandler([userDate]) {
    // объект даты без времени заводить нельзя
